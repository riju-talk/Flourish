...
// Full today's-tasks response, including real completion counts for the dashboard's
// Daily Rituals progress bar (getTodayTasks() above only exposes the pending list).
export async function getTodayProgress() {
  const { data } = await api.get('/tasks/today');
  return data as { tasks: any[]; date: string; completed_count: number; total_count: number; completion_percent: number };
}
