...
- **Field naming note:** `01-PRD.md` §7 shows illustrative, product-level JSON examples
  ...
  implementation-accurate schema — it keeps the field names the running code actually
  reads/writes (`user_id`, `name`, `plant_type`, `sunlight_requirement`, ...). When in
...
| agent_profile | object | maintained by GroqService — personalization context (see below) |
| created_at / updated_at | timestamp | server timestamps |

### `privacy` object (sub-field of `profiles`)
Governs what other authenticated users can see about you. **Privacy-safe by default —
this is the standing product decision** (see `01-PRD.md` §5.10 and
